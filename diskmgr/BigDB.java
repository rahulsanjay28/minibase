...
public class BigDB implements GlobalConst {
    ...
    public void add_file_entry(String fname, PageId start_page_num)
        ...
        do {// Start DO01
            ...
            if (hpid.pid == 0) {
                dp = new BigDBFirstPage();
                ((BigDBFirstPage) dp).openPage(apage);
            } else {
                dp = new BigDBDirectoryPage();
                ((BigDBDirectoryPage) dp).openPage(apage);
            }
...
/**
 * interface of bigPageUsedBytes
 */
interface bigPageUsedBytes {
 ...
 */
class BigDBHeaderPage implements bigPageUsedBytes, GlobalConst {
     ...
     * Default constructor
     */
    public BigDBHeaderPage() {
    }

    /**
     * Constrctor of class BigDBHeaderPage
     *
     * @param page          a page of Page object
     * @param pageusedbytes number of bytes used on the page
     * @throws IOException
     */
    public BigDBHeaderPage(Page page, int pageusedbytes)
            throws IOException {
        data = page.getpage();
...
/**
 * BigDBFirstPage class which is a subclass of DBHeaderPage class
 */
class BigDBFirstPage extends BigDBHeaderPage {

    protected static final int NUM_DB_PAGE = MINIBASE_PAGESIZE - 4;
    protected static final short DISTINCT_ROW_COUNT = NUM_DB_PAGE - 2;
    protected static final short DISTINCT_COLUMN_COUNT = DISTINCT_ROW_COUNT - 2;

    /**
     * Default construtor
     */
    public BigDBFirstPage() {
        super();
    }

    /**
     * Constructor of class BigDBFirstPage class
     *
     * @param page a page of Page object
     * @throws IOException I/O errors
     */
    public BigDBFirstPage(Page page)
            throws IOException {
    ...
    public void setDistinctRowCount(int num)
            throws IOException {
        Convert.setIntValue(num, DISTINCT_ROW_COUNT, data);
    }

    public void setDistinctColumnCount(int num)
            throws IOException {
        Convert.setIntValue(num, DISTINCT_COLUMN_COUNT, data);
    }

    public int getDistinctRowcount()
            throws IOException {

        return (Convert.getIntValue(DISTINCT_ROW_COUNT, data));
    }

    public int getDistinctColumncount()
            throws IOException {

        return (Convert.getIntValue(DISTINCT_COLUMN_COUNT, data));
    }
...
/**
 * BigDBDirectoryPage class which is a subclass of DBHeaderPage class
 */
class BigDBDirectoryPage extends BigDBHeaderPage { //implements PageUsedBytes

    /**
     * Default constructor
     */
    public BigDBDirectoryPage() {
        super();
    }

    /**
     * Constructor of BigDBDirectoryPage class
     *
     * @param page a page of Page object
     * @throws IOException
     */
    public BigDBDirectoryPage(Page page)
            throws IOException {
